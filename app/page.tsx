...
const roleIdentity: Record<string, { name: string; initials: string }> = {
  ...
  "Super Admin": { name: "Siti Rahma", initials: "SA" },
};
    ...
    <>
      ...
      <aside className={`sidebar ${mobileOpen ? "is-open" : ""}`}>
        ...
        <nav className="sidebar-nav" aria-label="Navigasi utama">
          ...
          {navItems.slice(7).filter((item) => role === "Super Admin" || item.label !== "Manajemen Data").map((item) => {
            const Icon = item.icon;
            return (
              <button
                key={item.label}
        ...
        <div className="profile-wrap">
          ...
          {profileOpen ? (
            <div className="profile-menu">
              ...
              {["RM Funding", "Lead RM Funding", "Pemimpin Cabang", "Super Admin"].map((item) => (
                <button
                  key={item}
                  onClick={() => {
                    setRole(item);
  ...
  return (
    <>
      <section className="admin-heading">
        <div className="admin-title-icon"><UserCog size={24} /></div>
        <div><span>SUPER ADMIN · AKSES TERTINGGI</span><h1>Manajemen Data & Retensi</h1><p>Kontrol kapasitas, backup, penghapusan, pengguna, dan audit seluruh aplikasi.</p></div>
        <button className="secondary-button"><RefreshCw size={16} />Perbarui estimasi</button>
      </section>

      <div className="admin-alert"><ShieldCheck size={18} /><p><strong>Prinsip least privilege tetap berlaku.</strong> Super Admin mengatur sistem dan kebijakan. Akses isi foto sensitif harus tercatat dan dapat dibatasi melalui audit.</p></div>
      ...
      <section className="admin-grid">
        <article className="panel storage-estimator">
          ...
          <div className="estimator-controls">
            <label><span>RM aktif</span><input type="number" min="1" max="100" value={rmCount} onChange={(event) => setRmCount(Math.max(1, Number(event.target.value) || 1))} /><em>orang</em></label>
            <label><span>Kunjungan / RM / hari</span><input type="number" min="1" max="15" value={visitsPerDay} onChange={(event) => setVisitsPerDay(Math.max(1, Number(event.target.value) || 1))} /><em>kunjungan</em></label>
            <label><span>Foto / kunjungan</span><input type="number" min="1" max="5" value={photosPerVisit} onChange={(event) => setPhotosPerVisit(Math.max(1, Number(event.target.value) || 1))} /><em>foto</em></label>
            <label><span>Rata-rata foto</span><input type="number" min="100" max="1000" step="50" value={photoKb} onChange={(event) => setPhotoKb(Math.max(100, Number(event.target.value) || 100))} /><em>KB</em></label>
          </div>
      ...
      <section className="panel lifecycle-panel">
        ...
        <div className="lifecycle-track">
          ...
          <ChevronRight size={20} />
          <div className="lifecycle-item purge"><span>&gt;24 bulan</span><strong>Kandidat hapus</strong><p>Hanya setelah backup tervalidasi dan dua persetujuan.</p><em>30 hari recycle</em></div>
        </div>
        <div className="delete-guardrail"><CircleAlert size={18} /><p><strong>Jangan hapus otomatis berdasarkan umur saja.</strong> Legal hold, investigasi, audit, dan kebijakan retensi bank harus selalu mengalahkan jadwal penghapusan.</p></div>
      </section>
      ...
      <section className="admin-bottom-grid">
        ...
        <article className="panel safe-delete-panel">
          ...
          <ol><li><span>1</span><p><strong>Ekspor</strong><em>Download arsip + manifest</em></p></li><li><span>2</span><p><strong>Verifikasi</strong><em>Checksum dan jumlah berkas</em></p></li><li><span>3</span><p><strong>Persetujuan ganda</strong><em>Super Admin + Pinca</em></p></li><li><span>4</span><p><strong>Recycle 30 hari</strong><em>Dapat dipulihkan sebelum purge</em></p></li></ol>
      ...
      <section className="superadmin-scope">
        <div><UserCog size={20} /><p><strong>24 pengguna</strong><span>Aktif di 4 kelompok peran</span></p><button>Kelola akun</button></div>
        <div><ShieldCheck size={20} /><p><strong>4 role & 31 izin</strong><span>RBAC dan maker-checker</span></p><button>Kelola akses</button></div>
        <div><FileChartColumnIncreasing size={20} /><p><strong>1.842 audit event</strong><span>30 hari terakhir</span></p><button>Lihat audit</button></div>
      </section>

      {message ? <div className="admin-toast"><CircleCheckBig size={17} />{message}<button onClick={() => setMessage("")} aria-label="Tutup pemberitahuan"><X size={15} /></button></div> : null}
    </>
  );
}
...
function MobileBottomNav({ active, role, onChange, onAddVisit, onMore }: { active: NavKey; role: string; onChange: (key: NavKey) => void; onAddVisit: () => void; onMore: () => void }) {
  return (
    <nav className="mobile-bottom-nav" aria-label="Navigasi mobile">
      ...
      {role === "Super Admin" ? <button className={active === "Manajemen Data" ? "active" : ""} onClick={() => onChange("Manajemen Data")}><Database size={20} /><span>Data</span></button> : <button onClick={onMore}><Menu size={20} /><span>Lainnya</span></button>}
    </nav>
  );
}
...
export default function Home() {
  ...
  function changeRole(nextRole: string) {
    ...
    if (nextRole === "Super Admin") {
      setActive("Manajemen Data");
    } else if (active === "Manajemen Data") {
      setActive("Kunjungan");
    }
  }
